	...
	for j = 1:length(GAMMA)
		for z = 1:length(LAMBDA)
			for it = 1:3
				if it==1
				tic
				[P, Y_hat, Y, S, alpha, obj] = CPAS_updaSv(X, S_v, BETA(a), GAMMA(j),LAMBDA(z), options,0,[],[],[],[]);
				time = toc;
				else
				[P, Y_hat, Y, S, alpha, obj] = CPAS_updaSv(X, S_v, BETA(a), GAMMA(j),LAMBDA(z), options,1,P,Y_hat,Y,S);	
				end
			end
...
xlswrite('YaleA_3view_Ini.xlsx',RESULT);
