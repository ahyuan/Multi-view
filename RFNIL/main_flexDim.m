...
path = "F:/end2endMVC/data/YaleA_3view.mat"
...
for a = 1:length(BETA)
	for j = 1:length(GAMMA)
		for z = 1:length(LAMBDA)
			for k = 1:length(kDim)
				tic
				[P, Y_hat, Y, S, alpha, obj] = CPAS_flexDim(X,kDim(k),S_v, BETA(a), GAMMA(j),LAMBDA(z), options);
				time = toc;
				...
				for i_fea = fea_rate
                    ...
                    Dis = squareform(pdist(new_fea','squaredeuclidean'));
	                    Dis = -1.*Dis./2;
	                    S_temp = exp(Dis);
	                    rowSums = sum(S_temp,2);
	                    rowSum_v = repmat(rowSums,1,size(S_temp,2));
	                    S_temp = S_temp./rowSum_v;
                        S_fea = (S_temp+S_temp')/2;
					for i = 1:runtimes
% 						Label = litekmeans(new_fea', c,'Replicates',5); %label by k-means
%                         [Label,~] = run_kmeans(new_fea,c);

                        
                        Label = SpectralClustering(S_fea,c);
					...
					count = count+1;
				disp(count);
				RESULT(count,:) = [BETA(a),GAMMA(j),LAMBDA(z),kDim(k),sigma,i_fea,ACCmean,ACCstd,NMImean,NMIstd,Fmean,Fstd,ARmean,ARstd,Pumean,Pustd,Rmean,Rstd,Premean,Prestd,time];
				disp(RESULT(count,:))
				end
...
xlswrite('YaleA_3view-Spec-dims.xlsx',RESULT);
